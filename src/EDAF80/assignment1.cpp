...
int main()
{
	...
	while (!glfwWindowShouldClose(window)) {
		...
		std::stack<struct CelestialBodyRef> renderStack;

		CelestialBodyRef rootNode;
		rootNode.body = &sun;
		rootNode.parent_transform = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, 0.0f));
		renderStack.push(rootNode);

		while (!renderStack.empty())
		{
			CelestialBodyRef curentRef = renderStack.top();
			CelestialBodyRef childRef;
			childRef.parent_transform = curentRef.body->render(animation_delta_time_us, camera.GetWorldToClipMatrix(), curentRef.parent_transform, show_basis);

			
			renderStack.pop();
			for (auto& elem : curentRef.body->get_children())
			{
				childRef.body = elem;
				renderStack.push(childRef);
			}
		}
